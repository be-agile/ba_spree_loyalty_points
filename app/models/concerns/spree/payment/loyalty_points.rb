  ...
  class Payment
    module LoyaltyPoints
      ...
      def redeemable_loyalty_points_balance?
        order.user.loyalty_points_balance >= Spree::Store.default.preferred_loyalty_points_redeeming_balance
      end

      def redeemable_user_balance
        if order&.user.blank?
          errors.add :loyalty_points_balance, " not available for guest user"
        elsif !redeemable_loyalty_points_balance?
          min_balance = Spree::Store.default.preferred_loyalty_points_redeeming_balance
          errors.add :loyalty_points_balance, "should be at least #{min_balance.to_s + " " + "point".pluralize(min_balance)} for redeeming Loyalty Points"
        end
      end
    end
  end
